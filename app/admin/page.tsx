...
// අලුතින් Gallery එක සඳහා හැදූ Interface එක
...
export default function AdminPage() {
        ...
        try {
            if (editingId) {
                ...
                await addDoc(collection(db, "projects"), {
                    title, category, description, imageUrl, createdAt: serverTimestamp()
                });
    ...
    // Gallery Submit (අලුත් ෆන්ක්ශන් එක)
    const handleGallerySubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setGalleryStatus({ text: "Image added successfully! 🖼️", type: "success" });
            setGalleryTitle("");
            setGalleryImageUrl("");
            fetchGallery();
            ...
            setGalleryStatus({ text: "Failed to add image.", type: "error" });
        }
    };
        ...
        <div className="min-h-screen bg-gray-950 text-white p-6 md:p-12">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="space-y-16">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <div className="lg:col-span-1 bg-gray-900 border border-gray-800 p-8 rounded-2xl shadow-xl h-fit transition-all duration-300" style={editingId ? { borderColor: '#ca8a04' } : {}}>
                                ...
                                <form onSubmit={handleSubmit} className="space-y-5">
                                    <div>
                                        ...
                                        <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} required className="w-full bg-black/50 border border-gray-700 rounded-lg p-3 text-white focus:border-yellow-500 focus:outline-none" />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-semibold text-gray-400 mb-2">Category</label>
                                        <input type="text" value={category} onChange={(e) => setCategory(e.target.value)} required className="w-full bg-black/50 border border-gray-700 rounded-lg p-3 text-white focus:border-yellow-500 focus:outline-none" />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-semibold text-gray-400 mb-2">Image URL</label>
                                        <input type="url" value={imageUrl} onChange={(e) => setImageUrl(e.target.value)} required className="w-full bg-black/50 border border-gray-700 rounded-lg p-3 text-white focus:border-yellow-500 focus:outline-none" />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-semibold text-gray-400 mb-2">Description</label>
                                        <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} required className="w-full bg-black/50 border border-gray-700 rounded-lg p-3 text-white focus:border-yellow-500 focus:outline-none"></textarea>
                                    </div>
                            ...
                            <div className="lg:col-span-2 bg-gray-900 border border-gray-800 p-8 rounded-2xl shadow-xl">
                                ...
                                {loadingProjects ? (
                                    ...
                                    <div className="text-center text-gray-500 py-10 bg-black/30 rounded-xl border border-gray-800">No projects published yet.</div>
                                ) : (
                    ...
                    {/* =========================================
                        GALLERY SECTION (අලුතින් එකතු කළ කොටස)
                    ========================================= */}
                    <div className="border-t border-gray-800 pt-10">
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            {/* වම් පැත්ත: Add Gallery Image Form */}
                            <div className="lg:col-span-1 bg-gray-900 border border-gray-800 p-8 rounded-2xl shadow-xl h-fit">
                                <h2 className="text-xl font-bold mb-6 text-purple-400">🖼️ Add to Gallery</h2>
                                <form onSubmit={handleGallerySubmit} className="space-y-5">
                                    <div>
                                        <label className="block text-sm font-semibold text-gray-400 mb-2">Image Title / Caption</label>
                                        <input type="text" value={galleryTitle} onChange={(e) => setGalleryTitle(e.target.value)} required className="w-full bg-black/50 border border-gray-700 rounded-lg p-3 text-white focus:border-purple-500 focus:outline-none" placeholder="e.g. Agency Team at Work" />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-semibold text-gray-400 mb-2">Image URL</label>
                                        <input type="url" value={galleryImageUrl} onChange={(e) => setGalleryImageUrl(e.target.value)} required className="w-full bg-black/50 border border-gray-700 rounded-lg p-3 text-white focus:border-purple-500 focus:outline-none" placeholder="https://example.com/image.jpg" />
                                    </div>
                                    <button type="submit" className="w-full text-white font-bold py-3 rounded-lg bg-purple-600 hover:bg-purple-500 transition-colors mt-2">
                                        Upload to Gallery
                                    </button>
                            ...
                            <div className="lg:col-span-2 bg-gray-900 border border-gray-800 p-8 rounded-2xl shadow-xl">
                                ...
                                {loadingGallery ? (
                                    ...
                                    <div className="text-center text-gray-500 py-10 bg-black/30 rounded-xl border border-gray-800">No images in gallery yet.</div>
                                ) : (
